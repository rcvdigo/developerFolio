    ...
    <Fade bottom duration={1000} distance="20px">
      <div>
        <div
          ...
          onClick={() =>
            openUrlInNewTab(
              repo?.node?.url ?? repo.url,
              repo?.node?.name ?? repo.name
            )
          }
        >
          <div className="repo-name-div">
            ...
            <p className="repo-name">{repo?.node?.name ?? repo.name}</p>
          </div>
          <p className="repo-description">
            {repo?.node?.description ?? repo.description}
          </p>
          <div className="repo-stats">
            <div className="repo-left-stat">
              ...
              <span>
                ...
                <p>{repo?.node?.forkCount ?? repo.forkCount}</p>
              </span>
              <span>
                <svg
                  aria-hidden="true"
                ...
                <p>
                  {repo?.node?.stargazers?.totalCount ?? repo.stargazersCount}
                </p>
              </span>
            </div>
            <div className="repo-right-stat">
              <p>
                {formatFileSizeDisplay(repo?.node?.diskUsage ?? repo.diskUsage)}
              </p>
            </div>
          </div>
        </div>
